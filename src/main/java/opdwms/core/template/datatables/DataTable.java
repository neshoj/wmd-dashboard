 ...
 *
 * <p>This class responsible for allowing datatables to fetch information from the web server as
 * opposed to fetching all the information at once and then showing the table using datatables.
 *
 * <p>This class uses the HQL select syntax identified as follows: select_statement :: =
...
public class DataTable implements DatatablesInterface {
  ...
  @Autowired private HttpServletRequest _request;
  ...
  public Map<String, Object> showEsTable() {
    ...
    if (ObjectUtils.isEmpty(s)) {
      ...
      for (String field : _esDocFields) {
        if (!Arrays.asList(_esDateFields).contains(field)) {
          boolBuilder =
              boolBuilder.should(QueryBuilders.wildcardQuery(field, "*".concat(s).concat("*")));
        }
      }
    ...
    sourceBuilder
        ...
        .size(limit)
        .sort(
            new FieldSortBuilder(_esDocFields[sortFieldIndex])
                .order(SortOrder.fromString(sortDir)));
    ...
    try {
      ...
      for (SearchHit hit : searchHits) {
        ...
        parser.setTimeZone(TimeZone.getTimeZone("EAT"));
  ...
  /**
   * Specify the columns that will appear in the final result set in order to assist this class in
   * building the information needed to render the datatable result map
   *
   * @param select
   * @return DatatablesInterface
   */
  ...
  public DataTable select(String select) {
    ...
    for (String col : select.split(",")) {
      ...
      if (_selectParams.length() > 0) _selectParams.append(", ").append(col);
  ...
  /**
   * Specify the tables where the information will be fetched from i.e. the parent table and all the
   * respective joins
   *
   * @param from
   * @return DatatablesInterface
  ...
  public DataTable from(String from) {
    // Set the table stuff
    if (_fromParams.length() > 0) _fromParams.append(" ").append(from);
  ...
  /**
   * Specify the conditions that will be applied to the query. This will help in building the filter
   * used by datatables
   *
   * @param where
   * @return DatatablesInterface
  ...
  /**
   * Apply the group by clause in order to properly support the aggregate functions
   *
   * @param groupBy
   * @return DatatablesInterface
   */
  ...
  /**
   * Set the formatter that will be used to format the response generated by the class
   *
   * @param formatter
   * @return DatatablesInterface
   */
  ...
  public DataTable setFormatter(RowFormatInterface formatter) {
    ...
    if (!_formatChain.contains(formatter)) _formatChain.add(formatter);
  ...
  /**
   * Set the parameter bound to the parameterised query passed in the conditions
   *
   * @param key
   * @param value
   * @return DatatablesInterface
  ...
  public DataTable setParameters(Map<String, Object> map) {
    for (Map.Entry<String, Object> p : map.entrySet()) _params.put(p.getKey(), p.getValue());
  ...
  public Map<String, Object> showTable() {
    ...
    // When formatting the row
      ...
      for (Object[] row : buildResultSet("")) {
        ...
        for (RowFormatInterface fmt : _formatChain) row = fmt.formatRow(row);
  ...
  public Map<String, Object> showTable(Function<Object[], Object[]> func) {
    ...
    // When formatting the row
      ...
      for (Object[] row : buildResultSet("")) {
        ...
        row2 = func.apply(row);
        // Loop through the chain
        for (RowFormatInterface fmt : _formatChain) row2 = fmt.formatRow(row2);
  ...
  private List<Object[]> buildResultSet(String setting) {
    ...
    Query q =
        _nativeSQL
            ? session.createNativeQuery(hql.toString())
            : session.createQuery(hql.toString());

    if (!setting.equals("filtered-total")
        && !setting.equals("total")
        && !setting.equals("footer-totals")) {
  ...
  private StringBuilder buildHQL(String setting) {
    ...
    if (hasAggregateFunctions(filterParams)) {
      ...
      if (!StringUtils.isEmpty(filterParams)) hql.append(" HAVING ").append(filterParams);
    }
    ...
    // When using the WHERE clause
      ...
      if (!StringUtils.isEmpty(groupByStatement)) hql.append(groupByStatement);
    }
    ...
    if (!setting.equals("filtered-total")
        ...
        && !setting.equals("footer-totals")) {
      ss = setOrder();
      if (null != ss && ss.length() > 0) hql.append(" ORDER BY ").append(ss);
    }
      ...
      if (!s.isEmpty()) {
        for (int i = 0; i < Integer.parseInt(_request.getParameter("iColumns")); i++) {
          if (_request.getParameter("bSearchable_" + i).equals("true")) {
            ...
            sb.append("lower(")
                .append(_columnNames.get(i))
                .append(")")
                .append(" LIKE lower(:dtSearch)");
          }
        }
  ...
  private boolean checkColumn(String column) {
    ...
    return !(temp.startsWith("MIN")
        || temp.startsWith("MAX(")
        || temp.startsWith("SUM(")
        || temp.startsWith("COUNT("));
  }
  ...
  /**
   * Set the aux conditions specified in the request parameters used to limit and order the result
   * set
   *
   * @return String
   */
  private StringBuilder setOrder() {
    ...
    if (_request.getParameter("iSortCol_0").isEmpty()) return null;
