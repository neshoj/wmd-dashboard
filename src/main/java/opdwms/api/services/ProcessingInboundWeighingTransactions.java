...
public class ProcessingInboundWeighingTransactions implements ProcessingInboundWeighingTransactionsInterface {
    ...
    public Map<String, Object> saveRawWeighingTransaction(WeighbridgeTransactionsRequest request) {
        ...
        Optional<WeighbridgeStations> optionalWBSByCode = weighbridgeStationsServiceInterface.findByWBSId(request.getStationCode());
        ...
        if (optionalWBSByCode.isPresent()) {
            try {
                WeighingTransactions transaction = new WeighingTransactions()
                        ...
                        .setWbtStatus(request.getWBT_Status())
    ...
    private Map<String, Object> performOverloadCalculations(WeighingTransactions transaction) {
        ...
        Optional<AxleClassification> axleClassificationRepositoryByAxleCode = axleClassificationRepository.findByAxleCode(transaction.getAxleConfiguration().trim().toUpperCase().replaceAll("\\s",""));

        if (axleClassificationRepositoryByAxleCode.isPresent()) {

            AppSettings axle_weight_tolerance = settingsRepository.findByCode("AXLE_WEIGHT_TOLERANCE").get();
            ...
            switch (axleClassification.getAxleCount()) {
                case 2:
                    ...
                    weighedGrossVehicleWeight = transaction.getFirstAxleWeight().add(transaction.getSecondAxleWeight());
                    break;
                case 3:
                    transaction = handleThreeAxleRequest(transaction, axleClassification, percentageTolerance);
                    grossVehicleWeight = axleClassification.getAxleOne()
                            .add(axleClassification.getAxleTwo())
                            .add(axleClassification.getAxleThree());

                    weighedGrossVehicleWeight = transaction.getFirstAxleWeight()
                ...
                case 6:
                    ...
                    break;
                case 7:
                    transaction = handleSevenAxleRequest(transaction, axleClassification, percentageTolerance);
                    grossVehicleWeight = axleClassification.getAxleOne()
                            .add(axleClassification.getAxleTwo())
                            .add(axleClassification.getAxleThree())
                            .add(axleClassification.getAxleFour())
            ...
            // Check GVM overload
            transaction.setGvwExceededWeight(calculateAxleWeightsOverload(grossVehicleWeight, weighedGrossVehicleWeight))
                    .setGvwExceededPercent(calculateAxleWeightsPercentageOverload(grossVehicleWeight, weighedGrossVehicleWeight));
            // Transaction GVW overload
            transaction.setStatus(transaction.getGvwExceededWeight().intValue() > 0 ? GVM_OVERLOAD : GVM_WITHIN)
    ...
    private WeighingTransactions handleSevenAxleRequest(WeighingTransactions transaction, AxleClassification axleClassification, int percentageTolerance) {
        ...
        BigDecimal tolerableWeightAxleSeven = calculateTolerableWeight(axleClassification.getAxleSix(), percentageTolerance);
        transaction.setSeventhAxleWeightExceededValue(calculateAxleWeightsOverload(tolerableWeightAxleSeven, transaction.getSecondAxleWeight()));
        transaction.setSeventhAxleWeightExceededPercent(calculateAxleWeightsPercentageOverload(tolerableWeightAxleSeven, transaction.getSeventhAxleWeight()));

        return transaction;
    }

    private WeighingTransactions handleSixAxleRequest(WeighingTransactions transaction, AxleClassification axleClassification, int percentageTolerance) {

        transaction = handleFiveAxleRequest(transaction, axleClassification, percentageTolerance);

        BigDecimal tolerableWeightAxleSix = calculateTolerableWeight(axleClassification.getAxleSix(), percentageTolerance);
        transaction.setSixthAxleWeightExceededValue(calculateAxleWeightsOverload(tolerableWeightAxleSix, transaction.getSecondAxleWeight()));
        transaction.setSixthAxleWeightExceededPercent(calculateAxleWeightsPercentageOverload(tolerableWeightAxleSix, transaction.getSixthAxleWeight()));

        return transaction;
    }

    private WeighingTransactions handleFiveAxleRequest(WeighingTransactions transaction, AxleClassification axleClassification, int percentageTolerance) {

        transaction = handleFourAxleRequest(transaction, axleClassification, percentageTolerance);

        BigDecimal tolerableWeightAxleFive = calculateTolerableWeight(axleClassification.getAxleFive(), percentageTolerance);
        transaction.setFifthAxleWeightExceededValue(calculateAxleWeightsOverload(tolerableWeightAxleFive, transaction.getSecondAxleWeight()));
        transaction.setFifthAxleWeightExceededPercent(calculateAxleWeightsPercentageOverload(tolerableWeightAxleFive, transaction.getFifthAxleWeight()));

        return transaction;
    }

    private WeighingTransactions handleFourAxleRequest(WeighingTransactions transaction, AxleClassification axleClassification, int percentageTolerance) {

        transaction = handleThreeAxleRequest(transaction, axleClassification, percentageTolerance);

        BigDecimal tolerableWeightAxleFour = calculateTolerableWeight(axleClassification.getAxleFour(), percentageTolerance);
        transaction.setFourthAxleWeightExceededValue(calculateAxleWeightsOverload(tolerableWeightAxleFour, transaction.getSecondAxleWeight()));
        transaction.setFourthAxleWeightExceededPercent(calculateAxleWeightsPercentageOverload(tolerableWeightAxleFour, transaction.getFourthAxleWeight()));

        return transaction;
    }

    private WeighingTransactions handleThreeAxleRequest(WeighingTransactions transaction, AxleClassification axleClassification, int percentageTolerance) {

        transaction = handleTwoAxleRequest(transaction, axleClassification, percentageTolerance);

        BigDecimal tolerableWeightAxleThree = calculateTolerableWeight(axleClassification.getAxleThree(), percentageTolerance);
        transaction.setThirdAxleWeightExceededValue(calculateAxleWeightsOverload(tolerableWeightAxleThree, transaction.getSecondAxleWeight()));
        transaction.setThirdAxleWeightExceededPercent(calculateAxleWeightsPercentageOverload(tolerableWeightAxleThree, transaction.getThirdAxleWeight()));

        return transaction;
    }


    private WeighingTransactions handleTwoAxleRequest(WeighingTransactions transaction, AxleClassification axleClassification, int percentageTolerance) {
        ...
        BigDecimal tolerableWeight = calculateTolerableWeight(axleClassification.getAxleOne(), percentageTolerance);
        transaction.setFirstAxleWeightExceededValue(calculateAxleWeightsOverload(tolerableWeight, transaction.getFirstAxleWeight()));
        transaction.setFirstAxleWeightExceededPercent(calculateAxleWeightsPercentageOverload(tolerableWeight, transaction.getFirstAxleWeight()));

        BigDecimal tolerableWeightAxleTwo = calculateTolerableWeight(axleClassification.getAxleTwo(), percentageTolerance);
        transaction.setSecondAxleWeightExceededValue(calculateAxleWeightsOverload(tolerableWeightAxleTwo, transaction.getSecondAxleWeight()));
        transaction.setSecondAxleWeightExceededPercent(calculateAxleWeightsPercentageOverload(tolerableWeightAxleTwo, transaction.getSecondAxleWeight()));
     ...
     */
    private BigDecimal calculateAxleWeightsOverload(BigDecimal permissibleWeight, BigDecimal readAxleWeight) {
     ...
     */
    private BigDecimal calculateAxleWeightsPercentageOverload(BigDecimal permissibleWeight, BigDecimal readAxleWeight) {
        // -1 : if value of this BigDecimal is less than that of BigDecimal object passed as parameter.
        if (permissibleWeight.compareTo(readAxleWeight) < 0) {
            // Overload: Get overload weight
